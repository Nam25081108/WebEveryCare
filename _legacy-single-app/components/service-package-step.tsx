...
export function ServicePackageStep({ service, planId, onPlanChange, professionalConfig, onProfessionalChange, professionalError }: {
  ...
  return <div className="wizard-panel"><h2>Bạn cần dịch vụ nào?</h2><p>Chọn nhóm dịch vụ, sau đó cấu hình gói phù hợp với không gian.</p>
    {service.id !== "professional" ? <div><div className="booking-plans detailed-plans">{displayedPlans.map((item) => <div className={planId === item.id ? "booking-plan selected" : "booking-plan"} key={item.id} onClick={() => onPlanChange(item)} role="radio" aria-checked={planId === item.id} tabIndex={0}><input type="radio" name="plan" checked={planId === item.id} onChange={() => onPlanChange(item)}/><span><strong>{item.name}</strong><small>{item.description} • {item.meta}</small></span><b>{formatCurrency(item.price)}</b></div>)}</div>{groupWorkDetails && <button type="button" className="group-work-detail" onClick={() => setModal({ type:"work", title:service.name, details:groupWorkDetails })}><span><ListChecks/></span><span><strong>Chi tiết công việc</strong><small>Xem người dọn sẽ thực hiện những công việc gì tại từng phòng</small></span><ChevronRight/></button>}</div> : <div className="professional-config">
