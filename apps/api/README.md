...
Nếu đang có database cũ tên `sachnha`, hãy dừng backend, kết nối pgAdmin vào database `postgres`, rồi chạy file `database/rename-to-everycare.sql`. Dữ liệu cũ được giữ nguyên, chỉ đổi tên database.
...
dotnet tool run dotnet-ef database update --project apps\api\EveryCare.Api\EveryCare.Api.csproj --startup-project apps\api\EveryCare.Api\EveryCare.Api.csproj
```
...
dotnet run --project apps\api\EveryCare.Api\EveryCare.Api.csproj --launch-profile http
```
