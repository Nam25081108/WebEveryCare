...
public static class DatabaseSeeder
{
    ...
    private static async Task AddMissingUtilityServices(AppDbContext db, CancellationToken cancellationToken)
    {
        ...
        var hospitality = await db.ServiceGroups.IgnoreQueryFilters().Include(group => group.Packages)
        ...
        var hospitalityPackages = new (string Name, string Slug, decimal Price, int Minutes, decimal? Area)[]
        {
            ("Khách sạn/Homestay • Phòng đơn", "hotel-single", 90_000, 45, 15),
            ("Khách sạn/Homestay • Phòng đôi", "hotel-double", 130_000, 60, 25),
            ("Khách sạn/Homestay • Phòng gia đình", "hotel-family", 190_000, 90, 50),
            ("Khách sạn/Homestay • DORM dưới 8 giường", "hotel-dorm-under-8", 220_000, 120, null),
            ("Khách sạn/Homestay • DORM từ 8 giường", "hotel-dorm-8-plus", 300_000, 150, null),
            ("Căn hộ dịch vụ • 1 phòng ngủ", "apartment-1br", 250_000, 120, 50),
            ("Căn hộ dịch vụ • 2 phòng ngủ", "apartment-2br", 350_000, 180, 70),
            ("Căn hộ dịch vụ • 3 phòng ngủ", "apartment-3br", 450_000, 240, 100),
            ("Villa/Nhà nguyên căn • Phòng đơn", "villa-single", 100_000, 45, 15),
            ("Villa/Nhà nguyên căn • Phòng đôi", "villa-double", 140_000, 60, 25),
            ("Villa/Nhà nguyên căn • Phòng gia đình", "villa-family", 200_000, 90, 50)
        };
        for (var index = 0; index < hospitalityPackages.Length; index++)
        {
            var definition = hospitalityPackages[index];
            if (hospitality.Packages.Any(item => item.Slug == definition.Slug)) continue;
            var package = new ServicePackage
            {
                Name = definition.Name, Slug = definition.Slug,
                Description = definition.Area is null ? "Giá cho mỗi phòng" : $"Tối đa {definition.Area:0}m² • Giá cho mỗi phòng",
                Price = definition.Price, DurationMinutes = definition.Minutes, RequiredWorkers = 1,
                MaximumAreaSquareMeters = definition.Area, MaximumRooms = 1, DisplayOrder = index + 1, IsActive = true,
                ServiceGroup = hospitality
            };
            db.ServicePackages.Add(package);
        }
        ...
        var planned = new (string Category, string Slug, string Name, string Description, string Icon)[]
        {
            ...
            ("repair", "goi-tho-tai-nha", "Gọi thợ tại nhà", "Kết nối thợ phù hợp cho các công việc sửa chữa trong nhà.", "wrench"),
            ("cooling", "dien-lanh", "Điện lạnh", "Bảo trì, vệ sinh và sửa chữa thiết bị điện lạnh.", "air"),
            ("electronics", "dien-tu-gia-dung", "Điện tử gia dụng", "Kiểm tra và hỗ trợ thiết bị điện tử, gia dụng tại nhà.", "washer"),
            ("interior", "noi-that", "Nội thất", "Lắp ráp, sửa chữa và chăm sóc nội thất gia đình.", "home"),
            ("moving", "van-chuyen", "Vận chuyển", "Hỗ trợ vận chuyển đồ đạc và chuyển nhà theo nhu cầu.", "briefcase"),
            ("living", "sinh-hoat", "Sinh hoạt", "Các tiện ích hỗ trợ sinh hoạt gia đình hằng ngày.", "basket"),
            ("pets", "cham-soc-thu-cung", "Chăm sóc thú cưng", "Dịch vụ hỗ trợ chăm sóc thú cưng tại nhà.", "heart"),
            ("technology", "cong-nghe", "Công nghệ", "Hỗ trợ máy tính, mạng và thiết bị công nghệ tại nhà.", "wrench"),
            ("oddjobs", "viec-vat", "Việc vặt", "Hỗ trợ những công việc nhỏ phát sinh trong gia đình.", "sparkles"),
