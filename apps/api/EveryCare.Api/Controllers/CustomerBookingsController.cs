...
public sealed class CustomerBookingsController(AppDbContext db, BookingDispatchService dispatch, BookingLifecycleService lifecycle, PartnerAvailabilityService availability, ILogger<CustomerBookingsController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Review(Guid id, CreateReviewRequest request, CancellationToken cancellationToken)
    {
        ...
        if (comment?.Length > 500) return BadRequest(new { message = "Nhận xét không được vượt quá 500 ký tự." });
        var booking = await db.Bookings.Include(x => x.Review).Include(x => x.AssignedPartner).ThenInclude(x => x!.User)
            .SingleOrDefaultAsync(x => x.Id == id && x.Customer.Phone == NormalizePhone(request.Phone), cancellationToken);
        if (booking is null) return NotFound(new { message = "Không tìm thấy đơn hàng." });
        ...
        db.Reviews.Add(new Review
        {
            ...
            PartnerProfileId = booking.AssignedPartnerId.Value,
            Rating = request.Rating,
            Comment = comment
        });
        db.PartnerNotifications.Add(new PartnerNotification
        {
            PartnerUserId = booking.AssignedPartner!.UserId, BookingId = booking.Id, Type = "new_review",
            Title = $"Bạn vừa nhận đánh giá {request.Rating}/5 sao",
            Message = comment is null ? $"Khách hàng đã đánh giá {request.Rating}/5 sao cho lịch {booking.Code}." : $"Khách hàng đánh giá lịch {booking.Code}: “{comment}”"
        });
        try { await db.SaveChangesAsync(cancellationToken); }
        catch (DbUpdateException)
        {
            db.ChangeTracker.Clear();
            if (await db.Reviews.AsNoTracking().AnyAsync(x => x.BookingId == id, cancellationToken))
                return Conflict(new { message = "Bạn đã đánh giá lượt làm việc này." });
            throw;
        }
        ...
        await db.SaveChangesAsync(cancellationToken);
        return Ok(new { message = "Đã gửi đánh giá và nhận xét đến Tasker. Cảm ơn bạn!", Rating = request.Rating, Comment = comment, profile.AverageRating });
    }
