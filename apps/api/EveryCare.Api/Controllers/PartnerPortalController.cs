...
public sealed class PartnerPortalController(AppDbContext db, PartnerSessionService sessions, PartnerAvailabilityService availability, BookingDispatchService dispatch) : ControllerBase
{
    ...
    public async Task<IActionResult> GetInvitations(CancellationToken cancellationToken)
    {
        ...
        if (user is null) return Unauthorized();
        var profileId = await db.PartnerProfiles.Where(x => x.UserId == user.Id).Select(x => x.Id).SingleAsync(cancellationToken);
        var items = await db.BookingAssignments.AsNoTracking().Where(x => x.PartnerProfileId == profileId && x.Status != AssignmentStatus.Rejected)
    ...
    public async Task<IActionResult> Respond(Guid assignmentId, RespondInvitationRequest request, CancellationToken cancellationToken)
    {
        var user = await RequirePartnerAsync(cancellationToken);
        if (user is null) return Unauthorized();
        var assignment = await db.BookingAssignments.AsNoTracking()
            .Include(x => x.Booking).ThenInclude(x => x.RecurringContract)
            .Include(x => x.Booking).ThenInclude(x => x.ServiceGroup)
            .Include(x => x.PartnerProfile).ThenInclude(x => x.ServiceCapabilities)
            .SingleOrDefaultAsync(x => x.Id == assignmentId && x.PartnerProfile.UserId == user.Id, cancellationToken);
        ...
        var hasCapability = assignment.PartnerProfile.ServiceCapabilities.Any(x => x.ServiceGroupId == assignment.Booking.ServiceGroupId);
        if (!hasCapability || !PartnerEligibilityPolicy.IsEligible(assignment.PartnerProfile, assignment.Booking.ServiceGroup.Slug, assignment.Booking.RequiredWorkers))
        {
            await db.BookingAssignments.Where(x => x.Id == assignmentId && x.Status == AssignmentStatus.Invited)
                .ExecuteUpdateAsync(setters => setters.SetProperty(x => x.Status, AssignmentStatus.Released).SetProperty(x => x.RespondedAt, DateTimeOffset.UtcNow), cancellationToken);
            return Conflict(new { message = "Đơn này không phù hợp với dịch vụ đã đăng ký hoặc số người hiện có của bạn." });
        }
    ...
    public async Task<IActionResult> CancelAssignedBooking(Guid bookingId, CancelAssignedBookingRequest request, CancellationToken cancellationToken)
    {
        ...
        if (booking.Status is not (BookingStatus.Assigned or BookingStatus.PartnerTravelling))
            return BadRequest(new { message = "Đơn đã bắt đầu thực hiện hoặc không còn có thể hủy nhận." });
    ...
    public async Task<IActionResult> ConfirmAssignedBooking(Guid bookingId, CancellationToken cancellationToken)
    {
        ...
        if (booking is null) return NotFound(new { message = "Không tìm thấy đơn đã nhận." });
        if (booking.Status != BookingStatus.Assigned) return BadRequest(new { message = "Không thể xác nhận đơn ở trạng thái hiện tại." });
    ...
    public async Task<IActionResult> GetReviews(CancellationToken cancellationToken)
    {
        ...
        if (user is null) return Unauthorized();
        var profileId = await db.PartnerProfiles.Where(x => x.UserId == user.Id).Select(x => x.Id).SingleAsync(cancellationToken);
        var items = await db.Reviews.AsNoTracking().Where(x => x.PartnerProfileId == profileId)
            .OrderByDescending(x => x.CreatedAt).Take(100)
            .Select(x => new
            {
                x.Id, x.Rating, x.Comment, x.CreatedAt,
                BookingCode = x.Booking.Code,
                Service = x.Booking.ServiceGroup.Name,
                CustomerName = x.Booking.Customer.FullName.Substring(0, 1) + "***"
            }).ToListAsync(cancellationToken);
        return Ok(items);
    }
