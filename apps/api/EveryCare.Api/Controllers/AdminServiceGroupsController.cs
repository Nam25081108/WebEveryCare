...
public sealed class AdminServiceGroupsController(AppDbContext db) : ControllerBase
{
    ...
    {
        var groups = await db.ServiceGroups.AsNoTracking()
            .Include(x => x.Packages).Include(x => x.ProfessionalPricingRules)
        ...
        var items = groups.Select(group => new
        {
            ...
            PricingRules = group.ProfessionalPricingRules.OrderBy(x => x.BuildingType).ThenBy(x => x.BuildingCondition).ThenBy(x => x.MinimumAreaSquareMeters).Select(rule => new
            {
                rule.Id, rule.BuildingType, rule.BuildingCondition, rule.AreaTier, rule.MinimumAreaSquareMeters,
                rule.MaximumAreaSquareMeters, rule.FixedPrice, rule.PricePerSquareMeter, rule.FurnishedMultiplier, rule.IsActive
            })
        }).ToList();
        return Ok(items);
    }
